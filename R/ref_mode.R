...
ref_mode_to_ggplot <- function(str) {
  ...
  e <- paste0(e, " + theme(text=element_text(family=\"Chelsea Market\"), panel.background = element_rect(fill = colorspace::lighten(cld:::cp[1]))) + scale_x_continuous(labels = NULL, name = \"time\") + scale_y_continuous(labels = NULL)")
  return(e)
}
