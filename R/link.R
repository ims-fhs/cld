...
#' cld:::vars(cld, "belastung %->% Flexibilisierung der Arbeit %->% Private Dinge während der Arbeit %->% Belastung")
#' cld <- import("tests/testthat/mdl/test-partial-variable-names.mdl")
#' cld:::vars(cld, "some")
vars <- function(.data, chain) {
  var_names <- trimws(unlist(strsplit(chain, "%->%")))
  indexes <- sapply(var_names, function(i) grep(tolower(i), tolower(.data$label)))
  # indexes <- sapply(var_names, function(i) setNames(seq_along(tolower(.data$label)), tolower(.data$label))[[tolower(i), exact=FALSE]])
  print(class(indexes))
