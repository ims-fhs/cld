...
test_that("add_textual_description adds textual description to a division of elements", {
  ...
  expect_equal(cld1$division[nrow(cld1)], 2)
  expect_error(cld %>% describe(type = "unknown_type", description = ""))
})

test_that("add_ref_mode adds a ref mode in ggplot format to a division of elements", {
  cld <- import("mdl/burnout.mdl")
  ...
  expect_equal(cld1$description[nrow(cld1)], "ggplot() + geom_segment(aes(x = 0, y =0, xend = 10, yend = 10)) + theme(panel.background = element_rect(fill = cld:::cp[1])) + labs(title = \"\", x = \"\", y = \"\")")
  expect_equal(cld1$division[nrow(cld1)], 2)
